...
def send():
    if request.method == "POST":
        ...
        downpayment = float(request.form["downpayment"]) / 100
        term_annual = float(request.form["term"])
        interest = float(request.form["interestRate"])

        session['price'] = purchase_price
        session['downpayment'] = downpayment
        session['term'] = term_annual
...
def schedule():
    ...
    purchase_price = session.get('price')
    downpayment = session.get('downpayment')
    term_annual = session.get('term')
    ...
    hoa_fees = 0
    # Estimate Property Taxes at 2% of Home Value
    ...
    ltv = 1 - downpayment
    downpayment_amount = round((purchase_price * (1 - ltv)), 2)
    ...
    cash_to_close = downpayment_amount + total_closing_costs
    ...
    amortization_schedule_dict = amortization_schedule_df.to_dict(orient="list")

    schedule = amortization_schedule_dict
...
def equity():
    ...
    equity_df["home_equity_minus_downpayment_closing_costs"] = equity_df["home_equity_without_appreciation"] - downpayment_amount - going_in_costs
    equity_df["equity_from_appreciation"] = equity_df["home_value"] - purchase_price
    
    equity_dict = equity_df.to_dict(orient="list")
    equity = equity_dict
...
def cashflow():
    ...
    # print(amortization_schedule_dict)
    ...
    cashflow_dict = cashflow_df.to_dict(orient="list")
...
def wealth_created():
    ...
    wealth_created = wealth_created_dict

    return jsonify(wealth_created)


#-----------------------------

@app.route("/rate_of_return")
def rate_of_return():
    ...
    rate_of_return_dict = rate_of_return_df.to_dict(orient="list")
    rate_of_return = rate_of_return_dict

    return jsonify(rate_of_return)
...
def yearly():
    ...
    yearly_df_dict = yearly_df.to_dict(orient="list")
    yearly = yearly_df_dict
